...
from model.voxelnetvlad.pointPWC.nonLocalNet import NonLocalNetCost
LEAKY_RATE = 0.1
use_bn = False
...
# def pointutil(nsample, pos1_t, pos2_t):
...
class PointConvFlow(nn.Module):
    def __init__(self, nsample, in_channel, localfea, mlp, bn = use_bn, use_leaky = True, usenonlocal=False):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if self.usenonlocal:
            localfeature = self.nonlocalnet(xyz1, neighbor_xyz)
...
class flow_predictor(nn.Module):  # mlp predict flow weight
    ...
    def __init__(self, in_channel_mlp, mlp, bn=True ):
        ...
        for i, num_out_channel in enumerate(mlp):
            self.mlp_convs.append(nn.Conv2d(in_channel_mlp, 
                                                       num_out_channel, 
                                                       kernel_size=(1, 1), 
                                                       stride=(1, 1),
                                                       bn=bn,
    ...
    def forward(self, points_f1, upsampled_feat, cost_volume):
        ...
        if points_f1 == None:
            ...
            points_concat = torch.cat([points_f1, cost_volume, upsampled_feat], dim=1) # B,nchannel1+nchannel2,ndataset1
        
        elif upsampled_feat == None:
            points_concat = torch.cat([points_f1, cost_volume], dim=1) # B,nchannel1+nchannel2,ndataset1   [1,192,64]
        ...
        points_concat = points_concat.squeeze(2)
                    
        return points_concat                       #B, nchannel1, npoint
