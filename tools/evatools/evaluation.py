...
import tools.evatools.transformations as tr
from tools.evatools.pose_evaluation_utils import quat_pose_to_mat
...
class kittiOdomEval():
    ...
    def plotPath_2D_3_FORD(self, seq, poses_gt, poses_result, plot_path_dir):
        '''
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            if int(seq) >= 30:
                self.plotPath_2D_3_FORD(seq, poses_gt, poses_result, eva_seq_dir)
            else:
        ...
        # print ("-------------------------------------------------")
        return all_t_err, all_r_err

def evaluation(root , timedir, epoch, SQUE):
    parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')
    parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default=os.path.join(root + 'data/pred_world/', timedir, epoch),  help='Directory path of storing the odometry results')
    # parser.add_argument('--gt_dir',     type=str, default='E:/kyxz2018/gtloamorg/',  help='Directory path of the ground truth odometry')
    # parser.add_argument('--result_dir', type=str, default='E:/kyxz2018/loamorg/',              help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default='{0:02d}'.format(SQUE) +'_pred',      help='The sequences to be evaluated')
    ...
    trans, rota = pose_eval.eval(toCameraCoord=args.toCameraCoord)   # set the value according to the predicted results
    return trans[0], rota[0]

def loamevaluation(gtdir, preddir,SQUE):
    parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')
    # parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    # parser.add_argument('--result_dir', type=str, default=os.path.join(root + 'data/pred_world/', timedir+"-"+epoch),  help='Directory path of storing the odometry results')
    parser.add_argument('--gt_dir',     type=str, default=os.path.join('E:/kittiloam2/',gtdir),  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default=os.path.join('E:/kittiloam2/',preddir),              help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default=SQUE +'_pred',      help='The sequences to be evaluated')
    parser.add_argument('--toCameraCoord',   type=lambda x: (str(x).lower() == 'true'), default=False, help='Whether to convert the pose to camera coordinate')
...
def loamimuevaluation(root, preddir,SQUE):
    parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')
    # parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    # parser.add_argument('--result_dir', type=str, default=os.path.join(root + 'data/pred_world/', timedir+"-"+epoch),  help='Directory path of storing the odometry results')
    parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default=preddir,              help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default=SQUE +'_pred',      help='The sequences to be evaluated')
    parser.add_argument('--toCameraCoord',   type=lambda x: (str(x).lower() == 'true'), default=False, help='Whether to convert the pose to camera coordinate')
...
def gtlocalevaluation(root, SQUE, iseuler):
    if iseuler:
        resultdir = root + 'data/gt_local_euler_RMSE/'
    else:
        resultdir = root + 'data/gt_local_RMSE/'
    parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')
    parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default=resultdir,  help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default='{0:02d}'.format(SQUE) +'_pred',      help='The sequences to be evaluated')
    parser.add_argument('--toCameraCoord',   type=lambda x: (str(x).lower() == 'true'), default=False, help='Whether to convert the pose to camera coordinate')
...
def icplocalevaluation(root, icproot, icpdir, SQUE):
    resultdir = os.path.join(root, "data", icproot, icpdir + "_RMSE")
    parser = argparse.ArgumentParser(description='KITTI Evaluation toolkit')
    parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default=resultdir,  help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default='{0:02d}'.format(SQUE) +'_pred',      help='The sequences to be evaluated')
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--gt_dir',     type=str, default=root + 'data/gt_world/',  help='Directory path of the ground truth odometry')
    # parser.add_argument('--result_dir', type=str, default=os.path.join(root + 'data/pred_world/', timedir+"-"+epoch),  help='Directory path of storing the odometry results')
    parser.add_argument('--gt_dir',     type=str, default=os.path.join('A:/Unsupodo/data/gt_world/'),  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default=os.path.join('A:/Unsupodo/data/pred_world/syqnet/'), help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default='10_pred',      help='The sequences to be evaluated')
    parser.add_argument('--toCameraCoord',   type=lambda x: (str(x).lower() == 'true'), default=False, help='Whether to convert the pose to camera coordinate')
